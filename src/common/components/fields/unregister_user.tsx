...
import { useFetchEmployeesQuery, useFetchUsersQuery } from '../../../store';
...
const CustomUnregisterUserField = React.forwardRef<HTMLInputElement, CustomUserFieldProps>(({ label, value, onChange, error, helperText, ...rest }, ref) => {
    ...
    React.useEffect(() => {
        if (users && employees) {
            const Employee_Ids = employees.map(employee => employee.EmployeeId);
            setUsersList(
                users.filter(user => user.Email != "" && !Employee_Ids.includes(user.Id)).map(user => ({
                    id: user.Id,
                    label: user.Title,
                    email: user.Email
                }))
...
export default CustomUnregisterUserField;